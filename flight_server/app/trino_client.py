...
class TrinoClient:
    ...
    @monitor_performance
    def describe_table(self, table_name: str, catalog: Optional[str] = None, schema: Optional[str] = None) -> pd.DataFrame:
        """Describe table structure."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        with trace_operation("describe_table", {"table": f"{catalog}.{schema}.{table_name}"}):
            return self.execute_query(f"DESCRIBE {catalog}.{schema}.{table_name}")
    
    @monitor_performance
    def get_table_stats(self, table_name: str, catalog: Optional[str] = None, schema: Optional[str] = None) -> pd.DataFrame:
        """Get table statistics."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        with trace_operation("get_table_stats", {"table": f"{catalog}.{schema}.{table_name}"}):
            return self.execute_query(f"SHOW STATS FOR {catalog}.{schema}.{table_name}")
    
    @monitor_performance
    def get_table_partitions(self, table_name: str, catalog: Optional[str] = None, schema: Optional[str] = None) -> pd.DataFrame:
        """Get table partitions."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        with trace_operation("get_table_partitions", {"table": f"{catalog}.{schema}.{table_name}"}):
            return self.execute_query(f"SELECT * FROM {catalog}.{schema}.\"{table_name}$partitions\"")
    ...
    def create_table_as_select(self, 
                              ...
                              table_properties: Optional[Dict[str, str]] = None) -> bool:
        """Create table as select (CTAS)."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        
        with trace_operation("create_table_as_select", {
    ...
    def insert_into_table(self, 
        ...
        with trace_operation("insert_into_table", {
    ...
    @monitor_performance
    def optimize_table(self, table_name: str, catalog: Optional[str] = None, schema: Optional[str] = None) -> bool:
        """Optimize Iceberg table (compaction)."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        
        with trace_operation("optimize_table", {"table": f"{catalog}.{schema}.{table_name}"}):
            query = f"CALL {catalog}.system.rewrite_data_files('{schema}', '{table_name}')"
    ...
    def expire_snapshots(self, 
        ...
        with trace_operation("expire_snapshots", {
            "table": f"{catalog}.{schema}.{table_name}",
    ...
    @monitor_performance
    def get_table_history(self, table_name: str, catalog: Optional[str] = None, schema: Optional[str] = None) -> pd.DataFrame:
        """Get table history/snapshots."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        
        with trace_operation("get_table_history", {"table": f"{catalog}.{schema}.{table_name}"}):
            return self.execute_query(f"SELECT * FROM {catalog}.{schema}.\"{table_name}$history\"")
    ...
    def create_materialized_view(self, 
                                ...
                                catalog: Optional[str] = None, 
                                schema: Optional[str] = None,
                                refresh_interval: str = "1h") -> bool:
        """Create materialized view."""
        catalog = catalog or self.config.catalog
        schema = schema or self.config.schema
        
        with trace_operation("create_materialized_view", {
...
def get_trino_client(config: Optional[TrinoConfig] = None) -> TrinoClient:
...
def query_iceberg_table(table_name: str, 
    ...
    if query is None:
        query = f"SELECT * FROM {client.config.catalog}.{client.config.schema}.{table_name}"
        if limit:
            query += f" LIMIT {limit}"
