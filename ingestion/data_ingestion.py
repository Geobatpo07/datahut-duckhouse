    ...
    def validate_dataframe(self, df: pd.DataFrame) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if memory_mb > self.config.max_memory_mb:
                issues.append(
                    f"DataFrame size ({memory_mb:.2f}MB) exceeds limit ({self.config.max_memory_mb}MB)"
                )
...
class TenantDataManager:
    ...
    def add_tenant_metadata(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"Added tenant metadata for tenant: {self.config.tenant_id}"
            )
...
class DataIngestionOrchestrator:
    ...
    def _determine_target_engine(self, df: pd.DataFrame) -> TargetEngine:
        ...
        try:
            ...
            if data_size_mb > 500 or record_count > 1000000:  # Large dataset
                ...
                self.logger.info(
                    f"Large dataset detected ({data_size_mb:.2f}MB, {record_count} records) - routing to Trino"
                )
            else:  # Small dataset
                target = TargetEngine.DUCKDB
                self.logger.info(
                    f"Small dataset detected ({data_size_mb:.2f}MB, {record_count} records) - routing to DuckDB"
                )
    ...
    def _write_to_trino(self, df: pd.DataFrame, table_name: str) -> bool:
        ...
        try:
            ...
            insert_query = (
                f"INSERT INTO {catalog}.{schema}.{table_name} VALUES ({placeholders})"
            )
            with client.cursor() as cursor:
                cursor.executemany(insert_query, df.itertuples(index=False, name=None))

            client.close()
            self.logger.info(f"Data written to Trino table: {table_name}")
            return True
    ...
    def ingest_data(
        ...
        try:
            ...
            self.logger.info(
                f"Ingestion completed successfully. Records: {self.metrics.records_processed}"
            )
            return self.metrics
